...
class ProductController {
  ...
  static add = async (request, response) => {
    try {
      ...
      var data = await DBquery(
        `insert into p_product (pp_name , pp_code ,pg_id,pp_quantity,pp_flag) VALUES ('${reqBody.name}',upper('${reqBody.code}'),${reqBody.group_id},${reqBody.quantity},${reqBody.flag}) ` +
          `returning pp_id as id, pp_name as name, pp_code as code , pg_id as group_id, pp_quantity as quantity, pp_flag as flag;`
      );
      var m = new ResponseModel(data, data.length, 0);
      response.status(200).send(m);
    } catch (error) {
  ...
  static update = async (request, response) => {
    try {
      ...
      var data = await DBquery(
        `update p_product set pp_name = '${reqBody.name}' , pp_code = upper('${reqBody.code}') , pp_quantity = ${reqBody.quantity},pp_flag = ${reqBody.flag} ` +
          `returning pp_id as id, pp_name as name, pp_code as code , pg_id as group_id, pp_quantity as quantity, pp_flag as flag;`
      );
      var m = new ResponseModel(data, data.length, 0);
      response.status(200).send(m);
